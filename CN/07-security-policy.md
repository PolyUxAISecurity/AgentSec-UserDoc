...
> AgentSec Engine™ 是 AgentSec 的统一安全引擎。左侧导航展开后可访问四个子页面：**当前策略**、**规则实验室**、**AI 模型**、**策略市场**。
...
在左侧导航栏展开 **「AgentSec Engine™」**，点击 **「当前策略」**。
...
AgentSec Engine™ 包含四个子页面：

| 子页面 | 说明 |
|--------|------|
| **当前策略** | 查看和调整当前生效的安全规则（本页介绍） |
| **规则实验室** | 技术视角查看规则详情、编辑 JSON 源码 |
| **AI 模型** | 查看当前 LLM 模型配置（登录模式由服务端管理） |
...
点击 **「保存规则」** 将修改写入本地配置文件（`policy.json`）。**登录模式下**，策略由服务端统一管理，客户端会自动同步最新规则集。修改立即生效，下次分析就会使用新规则。
